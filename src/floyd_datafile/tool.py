...
Usage:
    ...
    $ echo '{foo:"bar"}' | python -m floyd_datafile
    {
        foo: bar,
    }
    $ echo '{foo:"bar"}' | python -m floyd_datafile --as-json
    {
        "foo": "bar"
    }
"""
...
def main(argv=None, host=None):
    ...
    if args.version:
        host.print(floyd_datafile.__version__)
        return 0
    ...
    obj = floyd_datafile.loads(inp)
    if args.as_json:
        s = json.dumps(obj, indent=args.indent)
    else:
        s = floyd_datafile.dumps(obj, indent=args.indent)
    host.print(s)
    return 0
...
def _parse_args(host, argv):
    ...
    parser.add_argument(
        ...
        '--version',
        action='store_true',
        help=f'show floyd_datafile library version ({floyd_datafile.__version__})',
    )
    parser.add_argument(
        '-c',
        metavar='STR',
